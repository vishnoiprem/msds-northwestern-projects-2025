...
```{r test3a}
# Try to load PASWR2 package and stream data
stream_loaded <- FALSE

# Attempt 1: Try to load from PASWR2
if (!requireNamespace("PASWR2", quietly = TRUE)) {
  cat("Installing PASWR2 package...\n")
  install.packages("PASWR2", repos = "http://cran.us.r-project.org", quiet = TRUE)
}
...
# Attempt 2: If PASWR2 doesn't work, create realistic stream data
if (!stream_loaded || !exists("stream")) {
  cat("Creating simulated stream data (PASWR2 not available)...\n\n")
  
  # Create realistic stream data similar to the PASWR2 dataset
  set.seed(123)
  n <- 20  # 20 observations per location
  
  stream <- data.frame(
    do = c(
      rnorm(n, mean = 8.5, sd = 1.2),   # Upstream (with canopy) - lower DO
      rnorm(n, mean = 11.5, sd = 1.5)   # Bottom (without canopy) - higher DO
    ),
    location = factor(rep(c("Upstream", "Bottom"), each = n))
  )
}
...
This is for graph and more . In (5)(d), we know the size of the table - 3 x 2 - and write a function to match. Often, though, we'll want to write functions that are flexible in some way.
